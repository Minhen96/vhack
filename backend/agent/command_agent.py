...
SYSTEM_PROMPT = (
    ...
    "  On battery_low       → do NOT call return_to_base. The drone (scanner OR delivery) returns automatically.\n"
    "                          Then wait_for_event.\n"
    ...
    "  On mission_stop      → the operator has requested a stop. End the mission immediately.\n"
    "                          Summarize all survivors found and aid delivered so far, then stop.\n"
    "  On timeout           → call wait_for_event again (searches still running).\n\n"

    "PHASE 4 — Sweep review (all drones are charged and idle at this point):\n"
    "  The mission runs continuously until a mission_stop event is received — never declare it complete on your own.\n"
    ...
    "  Else (any aborted OR survivors were found):\n"
    "    Go back to PHASE 2 immediately — drones are already charged — for continued coverage.\n\n"
