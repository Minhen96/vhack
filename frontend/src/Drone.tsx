...
export function Drone({ droneId }: DroneProps) {
  // 1. REFS: Physical & Visual Groups
  const groupRef = useRef<THREE.Group>(null);
  const droneRootRef = useRef<THREE.Group>(null); // TELEMETRY ROOT: Handled by CameraController
  ...
  // Cache survivor meshes to avoid scene traversal every frame
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToDronePosition(droneId, (drone: DroneState | null) => {
      ...
      // NOTE: The negation is needed to align backend azimuth with Three.js coordinate system.
      // If the drone appears to fly "backward" relative to its movement vector, add +Math.PI here.
  ...
  // Initial setup for layers and rotation order
  useEffect(() => {
  ...
  useFrame((state, delta) => {
    ...
    // 3. ROTATION (Handle wrap on root)
    ...
    // Applied banking and hover oscillation to visual only
    ...
    // 4. THROTTLED VISUALS (FOV / ALTITUDE)
    ...
    if (
      ...
      if (spotlightRef.current && spotlightTargetRef.current) {
        ...
        // Rotate target based on elevation (relative to drone nose)
        // Since model nose is at +Z, rotation happens around X
        // Elevation -45 should point target forward and down
        const targetDist = 20;
      ...
      if (coneRef.current) {
        ...
        // With the center positioned FORWARD of the nose, the apex is now perfectly anchored at the drone.
        ...
        // Offset center from the nose along the expansion vector (Forward & Down)
  ...
  return (
    <group ref={droneRootRef}>
      ...
      */}
      <group rotation={[0, Math.PI, 0]}> {/* FLIP THIS TO [0, 0, 0] IF DRONE FLIES BACKWARD */}
